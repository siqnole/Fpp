...
struct Emoji {
    std::string id;
    std::string name;
    std::optional<std::string> server_id;         // null = global / unicode
...
inline Emoji Emoji::from_json(const nlohmann::json& j) {
    ...
    if (j.contains("guild_id") && j["guild_id"].is_string()) e.server_id = j["guild_id"].get<std::string>();
    else if (j.contains("server_id") && j["server_id"].is_string()) e.server_id = j["server_id"].get<std::string>();
    ...
    if (j.contains("nsfw") && j["nsfw"].is_boolean()) e.nsfw = j["nsfw"].get<bool>();
    return e;
}
...
inline nlohmann::json Emoji::to_json() const {
    ...
    j["id"] = id;
    j["name"] = name;
    if (server_id) j["guild_id"] = *server_id;
