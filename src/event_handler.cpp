#include "fluxerpp/event_handler.h"
...
events::Message events::Message::from_json(const nlohmann::json& j) {
    ...
    if (j.contains("guild_id") && j["guild_id"].is_string()) {
        ev.server_id = j["guild_id"].get<std::string>();
    } else if (j.contains("server_id") && j["server_id"].is_string()) {
        ev.server_id = j["server_id"].get<std::string>();
    } else if (j.contains("server") && j["server"].is_string()) {
    ...
    if (j.contains("webhook_id") && j["webhook_id"].is_string()) {
        ev.author.bot = true;
        if (j.contains("author") && j["author"].is_object() && j["author"].contains("username")) {
            ev.author.username = j["author"]["username"].get<std::string>();
        }
    }

    if (j.contains("content")) ev.content = j["content"].get<std::string>();
    if (j.contains("nonce")) ev.nonce = j["nonce"].get<std::string>();
    if (j.contains("edited_timestamp") && !j["edited_timestamp"].is_null()) ev.edited = true;
    else if (j.contains("edited") && j["edited"].is_boolean()) ev.edited = j["edited"].get<bool>();
    ...
    if (j.contains("message_reference") && j["message_reference"].is_object()) {
        ...
        if (ref.contains("message_id") && ref["message_id"].is_string()) {
            ev.replies.push_back(ref["message_id"].get<std::string>());
        }
    ...
    if (j.contains("mentions") && j["mentions"].is_array()) {
        for (const auto& m : j["mentions"]) {
            ...
            else if (m.is_object() && m.contains("id") && m["id"].is_string()) ev.mentions.push_back(m["id"].get<std::string>());
        }
    }
