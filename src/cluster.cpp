#include "fluxerpp/cluster.h"
#include "fluxerpp/bot_module.h"
...
// Levenshtein helper for command suggestion
...
void cluster::start(bool return_after_init) {
    ...
    try {
        ...
        if (node.success()) {
            auto cfg = models::InstanceConfig::from_json(node.body);
            if (!cfg.ws.empty()) {
                config_.ws_url = cfg.ws;
                rest_.update_config(config_);
                utils::logger::log(LogLevel::INFO, "Discovered Gateway URL: " + cfg.ws, config_);
            }
        }
    } catch (...) {
        utils::logger::log(LogLevel::WARNING,
            "Could not fetch node info; using default URLs", config_);
    }
...
void cluster::on_message_delete(std::function<void(const events::MessageDelete&)> cb) { dispatcher_.on_message_delete(cb); }
void cluster::on_message_react(std::function<void(const events::MessageReact&)> cb) { dispatcher_.on_message_react(cb); }
void cluster::on_message_unreact(std::function<void(const events::MessageUnreact&)> cb) { dispatcher_.on_message_unreact(cb); }
void cluster::on_channel_create(std::function<void(const events::ChannelCreate&)> cb) { dispatcher_.on_channel_create(cb); }
void cluster::on_channel_update(std::function<void(const events::ChannelUpdate&)> cb) { dispatcher_.on_channel_update(cb); }
void cluster::on_channel_delete(std::function<void(const events::ChannelDelete&)> cb) { dispatcher_.on_channel_delete(cb); }
void cluster::on_server_create(std::function<void(const events::ServerCreate&)> cb) { dispatcher_.on_server_create(cb); }
void cluster::on_server_update(std::function<void(const events::ServerUpdate&)> cb) { dispatcher_.on_server_update(cb); }
void cluster::on_server_delete(std::function<void(const events::ServerDelete&)> cb) { dispatcher_.on_server_delete(cb); }
void cluster::on_server_member_join(std::function<void(const events::ServerMemberJoin&)> cb) { dispatcher_.on_server_member_join(cb); }
void cluster::on_server_member_leave(std::function<void(const events::ServerMemberLeave&)> cb) { dispatcher_.on_server_member_leave(cb); }
void cluster::on_server_member_update(std::function<void(const events::ServerMemberUpdate&)> cb) { dispatcher_.on_server_member_update(cb); }
void cluster::on_server_role_create(std::function<void(const events::ServerRoleCreate&)> cb) { dispatcher_.on_server_role_create(cb); }
void cluster::on_server_role_update(std::function<void(const events::ServerRoleUpdate&)> cb) { dispatcher_.on_server_role_update(cb); }
void cluster::on_server_role_delete(std::function<void(const events::ServerRoleDelete&)> cb) { dispatcher_.on_server_role_delete(cb); }
void cluster::on_user_update(std::function<void(const events::UserUpdate&)> cb) { dispatcher_.on_user_update(cb); }
void cluster::on_user_settings_update(std::function<void(const events::UserSettingsUpdate&)> cb) { dispatcher_.on_user_settings_update(cb); }
void cluster::on_user_presence_update(std::function<void(const events::UserPresenceUpdate&)> cb) { dispatcher_.on_user_presence_update(cb); }
void cluster::on_channel_start_typing(std::function<void(const events::ChannelStartTyping&)> cb) { dispatcher_.on_channel_start_typing(cb); }
void cluster::on_channel_stop_typing(std::function<void(const events::ChannelStopTyping&)> cb) { dispatcher_.on_channel_stop_typing(cb); }
void cluster::on_webhook_update(std::function<void(const events::WebhookUpdate&)> cb) { dispatcher_.on_webhook_update(cb); }
void cluster::on_voice_state_update(std::function<void(const events::VoiceStateUpdate&)> cb) { dispatcher_.on_voice_state_update(cb); }
void cluster::on_error(std::function<void(const events::Error&)> cb) { dispatcher_.on_error(cb); }
void cluster::on_logout(std::function<void(const events::Logout&)> cb) { dispatcher_.on_logout(cb); }
void cluster::on_raw_event(std::function<void(const std::string& type, const nlohmann::json& data)> cb) { dispatcher_.on_raw_event(cb); }
...
void cluster::begin_typing(const std::string& channel_id) { gateway_.begin_typing(channel_id); }
void cluster::end_typing(const std::string& channel_id) { gateway_.end_typing(channel_id); }
...
void cluster::remove_timeout(const std::string& server_id,
    ...
    std::thread([this, server_id, user_id, callback]() {
        try {
            nlohmann::json fields;
            fields["communication_disabled_until"] = nullptr;
            auto res = rest_.edit_member(server_id, user_id, fields);
            if (callback) callback(res.success());
        } catch (const std::exception& e) {
            utils::logger::log(LogLevel::ERROR,
...
void cluster::add_role_to_member(const std::string& server_id,
    ...
    std::thread([this, server_id, user_id, role_id, callback]() {
        try {
            auto patch_res = rest_.put("/guilds/" + server_id + "/members/" + user_id + "/roles/" + role_id);
            std::string err_msg;
            if (!patch_res.success()) {
                err_msg = patch_res.error_message();
            }
...
void cluster::remove_role_from_member(const std::string& server_id,
    ...
    std::thread([this, server_id, user_id, role_id, callback]() {
        try {
            auto patch_res = rest_.del("/guilds/" + server_id + "/members/" + user_id + "/roles/" + role_id);
            std::string err_msg;
            if (!patch_res.success()) {
                err_msg = patch_res.error_message();
            }
...
void cluster::set_slowmode(const std::string& channel_id,
    ...
    std::thread([this, channel_id, seconds, callback]() {
        try {
            nlohmann::json fields;
            fields["rate_limit_per_user"] = seconds;
...
void cluster::clear_slowmode(const std::string& channel_id,
                            std::function<void(bool)> callback) {
    set_slowmode(channel_id, 0, callback);
}

void cluster::update_status(const std::string& text,
                           const std::string& presence,
                           std::function<void(bool)> callback) {
    // Note: Discord updates status via the Gateway identify / status update payloads
    std::thread([this, text, presence, callback]() {
        try {
            // Send Gateway status update payload
            nlohmann::json status_obj = {
                {"op", 3},
                {"d", {
                    {"status", presence == "Idle" ? "idle" : (presence == "Invisible" ? "invisible" : "online")},
                    {"activities", nlohmann::json::array({{{"name", text}, {"type", 0}}})},
                    {"since", nullptr},
                    {"afk", false}
                }}
            };
            gateway_.send_raw(status_obj);
...
void cluster::fetch_channel_permissions(const std::string& channel_id,
    ...
    std::thread([this, channel_id, callback]() {
        try {
            auto res = rest_.get_channel(channel_id);
            if (res.success()) {
                if (callback) callback(res.body.value("permission_overwrites", nlohmann::json::array()), true);
            } else {
...
void cluster::setup_cache_listeners() {
    ...
    this->on_ready([this](const events::Ready& e) {
        ...
        utils::logger::log(LogLevel::DEBUG,
            ...
            ", servers=" + std::to_string(e.servers.size()), config_);
...
std::optional<models::Member> cluster::get_member(const std::string& server_id, const std::string& user_id) const {
    std::shared_lock<std::shared_mutex> lock(cache_mutex_);
...
size_t cluster::get_member_count_sync(const std::string& server_id) const {
    std::shared_lock<std::shared_mutex> lock(cache_mutex_);
    auto s_it = member_cache_.find(server_id);
    if (s_it != member_cache_.end()) {
...
size_t cluster::get_total_member_count() const {
    std::shared_lock<std::shared_mutex> lock(cache_mutex_);
    size_t total = 0;
...
void cluster::setup_default_help() {
    register_command("help", [this](cluster& bot, const events::Message& msg,
        ...
        embed["color"] = 5793010; // default color
        embed["title"] = "fluxerpp default help menu (page 1 of " + std::to_string(max_pages) + ")";
        ...
        bot.send_message(msg.channel_id, payload,
            [this, msg, max_pages](fluxerpp::models::Message res, bool success) {
    ...
    this->on_message_react([this](const events::MessageReact& e) {
        ...
        if (page_changed) {
            ...
            nlohmann::json embed = nlohmann::json::object();
            embed["color"] = 5793010;
            embed["title"] = "fluxerpp default help menu (page " +
